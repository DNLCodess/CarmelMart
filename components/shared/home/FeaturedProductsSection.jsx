...
function ProductCard({ product, rank, index }) {
  ...
  const originalPrice =
    product.salePrice || product.sale_price ? product.price : null;
    ...
    >
      <Link href={`/product/${product.id}`}>
        <div className="relative bg-white rounded-3xl overflow-hidden border border-gray-200 hover:shadow-xl hover:shadow-black/10 transition-all duration-300 hover:-translate-y-1">
          ...
          {rankStyle && (
            <div
              className={`absolute top-3 left-3 z-10 w-9 h-9 rounded-full ${rankStyle.badge} flex items-center justify-center text-xs font-black shadow-lg ${rankStyle.glow}`}
            >
              {rankStyle.label}
            </div>
          )}
