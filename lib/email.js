...
// ── Lazy transporter ──────────────────────────────────────────────────────────
// Created on first use so the module loads cleanly even when SMTP vars
// are not set (development dry-run mode).
...
/**
 * Sent to the logistics partner (rider/coordinator) when an order is assigned to them.
 * @param {{ to: string, partner: object, order: object, items: object[], assignedBy: string }} params
 */
export async function sendLogisticsAssignment({ to, partner, order, items, assignedBy }) {
  const shortId = `#CM-${(order.id ?? "").slice(0, 8).toUpperCase()}`;
  const addr    = order.delivery_address ?? {};
  ...
  const itemRows = (items ?? []).map((it) => `
    <tr>
      <td style="padding:8px 0;border-bottom:1px solid #f1f5f9;font-size:14px;color:#1e293b;">
        ${it.name ?? "Product"} &times;${it.quantity}
      </td>
      <td style="padding:8px 0;border-bottom:1px solid #f1f5f9;font-size:14px;font-weight:700;color:#0f172a;text-align:right;">
        ${ngn((it.unit_price ?? 0) * it.quantity)}
      </td>
    </tr>`).join("");

  const html = wrap(`Delivery Assignment — ${shortId} — CarmelMart`, `
    <h2 style="font-size:20px;font-weight:700;color:#0f172a;margin:0 0 6px;">🚚 New Delivery Assignment</h2>
    <p style="font-size:14px;color:#475569;margin:0 0 4px;">
      Hi <strong>${partner.contact_name ?? partner.name}</strong>,
    </p>
    <p style="font-size:14px;color:#475569;margin:0 0 24px;">
      A new order has been assigned to <strong>${partner.name}</strong> for delivery.
      Please arrange pickup from the vendor and deliver to the address below.
    </p>

    <!-- Order ID box -->
    <div style="background:#f8fafc;border-left:4px solid #560238;border-radius:8px;padding:14px 18px;margin-bottom:20px;">
      <p style="margin:0;font-size:13px;color:#64748b;">Order ID</p>
      <p style="margin:4px 0 0;font-size:20px;font-weight:800;color:#0f172a;letter-spacing:1px;">${shortId}</p>
    </div>

    <!-- Delivery address -->
    <div style="background:#fefce8;border:1px solid #fde68a;border-radius:12px;padding:18px 20px;margin-bottom:20px;">
      <p style="font-size:13px;font-weight:700;color:#92400e;margin:0 0 10px;">📍 Delivery Address</p>
      <table width="100%" cellpadding="0" cellspacing="0">
        ${infoRow("Customer", addr.fullName ?? "—")}
        ${infoRow("Phone", addr.phone ?? "—")}
        ${infoRow("Address", [addr.houseNumber, addr.street].filter(Boolean).join(" ") || "—")}
        ${addr.area ? infoRow("Area", addr.area) : ""}
        ${infoRow("City / LGA", [addr.city, addr.lga].filter(Boolean).join(", ") || "—")}
        ${infoRow("State", addr.state ?? "—")}
        ${addr.landmark ? infoRow("Landmark", addr.landmark) : ""}
        ${addr.delivery_instructions ? infoRow("Instructions", addr.delivery_instructions) : ""}
      </table>
    </div>

    <!-- Items -->
    <p style="font-size:13px;font-weight:700;color:#0f172a;margin:0 0 4px;">Items to Deliver</p>
    <table width="100%" cellpadding="0" cellspacing="0" style="border-top:1px solid #e2e8f0;margin-top:8px;margin-bottom:16px;">
      ${itemRows}
      <tr>
        <td style="padding:10px 0;font-size:14px;font-weight:700;color:#0f172a;">Total</td>
        <td style="padding:10px 0;font-size:15px;font-weight:800;color:#16a34a;text-align:right;">${ngn(order.total)}</td>
      </tr>
    </table>

    <p style="font-size:13px;color:#475569;margin:0 0 6px;">
      <strong>Payment:</strong> ${order.payment_method === "pod" ? "⚠️ Pay on Delivery — collect cash from customer" : "✅ Paid online — no cash collection needed"}
    </p>

    <div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:12px;padding:14px 18px;margin:20px 0;">
      <p style="margin:0;font-size:13px;color:#166534;">
        📞 Please confirm receipt of this assignment by replying to this email or contacting the CarmelMart logistics team.
        Update the team when pickup and delivery are completed.
      </p>
    </div>

    <p style="font-size:12px;color:#94a3b8;margin-top:8px;">
      Assigned by: ${assignedBy ?? "CarmelMart Logistics"}
    </p>
  `, { preheader: `New delivery assignment: ${shortId} to ${addr.city ?? ""}` });

  await send({
    to,
    subject: `🚚 Delivery Assignment — ${shortId} — CarmelMart`,
    html,
  });
}
  ...
  for (let i = 0; i < recipients.length; i += BATCH_SIZE) {
    ...
    const results = await Promise.allSettled(
      batch.map((email) => {
        ...
        return getTransporter().sendMail({ from: FROM, to: email, subject, html });
      })
    );
    results.forEach((r) => {
