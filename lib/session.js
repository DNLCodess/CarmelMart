...
export async function requireVendor() {
  ...
  // KYC wizard not yet completed — redirect to resume it
  if (!vendor.payment_verified) redirect("/vendor-kyc");

  // Payment done but awaiting admin approval (or rejected)
  if (vendor.verification_status !== "verified") redirect("/vendor-pending");

  return user;
}

/**
 * Requires accountant role (or admin — admins can always access the accountant portal).
 * Used to protect the /accountant/* portal.
 */
export async function requireAccountant() {
  const user = await getServerUser();
  if (!user) redirect("/login");
  if (user.role !== "accountant" && user.role !== "admin") redirect("/unauthorized");
  if (user.status === "suspended" || user.status === "banned") redirect("/suspended");
  return user;
}
