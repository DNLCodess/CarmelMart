...
import {
  ...
  Minus, Plus, ArrowLeft, Package, RefreshCw, Zap, Eye,
  MapPin, Clock, MessageCircle, ThumbsUp, BadgeCheck,
} from "lucide-react";
...
function RelatedCard({ product }) {
  ...
  const isWished  = wishlist.some((w) => w === product.id || w?.id === product.id);
              ...
              <div className="space-y-5">
                ...
                {product.vendor && (
                  ...
                  >
                    ...
                    {product.vendor.verified && (
                      <BadgeCheck className="w-4 h-4 text-blue-500" aria-label="Verified vendor" />
                    )}
                  </Link>
                )}
                ...
                <div className="bg-gray-50 rounded-2xl p-4 space-y-1">
                  ...
                  {price < 10000 && (
                    <p className="text-xs text-gray-500 flex items-center gap-1">
                      <Truck className="w-3.5 h-3.5" /> Add ₦{(10000 - price).toLocaleString()} more for free delivery
                    </p>
                  )}
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2 text-sm text-gray-600">
                    ...
                    <span>
                      Estimated delivery: <span className="font-semibold text-gray-900">{deliveryEstimate}</span>
                      {product.location && ` · Ships from ${product.location}`}
                    </span>
                  </div>
                  <div className="flex items-center gap-2 text-sm text-gray-600">
                    <RotateCcw className="w-4 h-4 text-primary shrink-0" />
                    <span>7-day easy returns — <span className="font-semibold text-gray-900">no questions asked</span></span>
                  </div>
                </div>
                    ...
                    {Object.entries(attrs).map(([key, vals]) => (
                      Array.isArray(vals) && vals.length > 0 && (
                        <AttributeSelector
                          ...
                          onChange={(v) => setSelectedAttrs((s) => ({ ...s, [key]: v }))}
                        />
                      )
                    ))}
                  </div>
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    ...
                    className={`p-3.5 rounded-full border-2 transition-colors ${
                      isWishlisted ? "border-red-400 bg-red-50 text-red-500" : "border-gray-200 text-gray-600 hover:border-red-300 hover:text-red-400"
                    }`}
                  ...
                  <div className="bg-white rounded-2xl border border-gray-100 p-4">
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm font-bold text-gray-900">{product.vendor.name}</p>
                          {product.vendor.verified && (
                    ...
                    <div className="flex items-center gap-4 text-xs text-gray-500">
                      <span className="flex items-center gap-1"><Clock className="w-3 h-3" /> Usually ships in 24–48 hrs</span>
                      <span className="flex items-center gap-1"><MessageCircle className="w-3 h-3" /> Responds within 2 hrs</span>
                    </div>
                  </div>
                )}
              </div>
                    ...
                    <table className="w-full text-sm">
                      <tbody className="divide-y divide-gray-100">
                        {Object.entries(attrs).map(([key, val]) => (
                          <tr key={key}>
                            <td className="py-3 pr-4 text-gray-500 font-medium capitalize w-40">{key}</td>
                            <td className="py-3 text-gray-900">{Array.isArray(val) ? val.join(", ") : String(val)}</td>
                          </tr>
                        ))}
                        {product.category && (
                          <tr>
                            <td className="py-3 pr-4 text-gray-500 font-medium w-40">Category</td>
                            <td className="py-3 text-gray-900">{product.category.name}</td>
                          </tr>
                        )}
                      </tbody>
                    </table>
